...
implicit none
    ...
    ! Input parameters
    integer ::threads,itr, nx, ny,nz,nt,i,j,k,my_rank,num_procs,p,start_idx,end_idx,ierr
...
if (my_rank==1) then
 ...
 !   Told = T
   ...
   ! real :: k1, k2, CFL
    ...
    k3 = alpha * (dt / (dz**2))
   ! CFL = k1 + k2
   OPEN(UNIT=100, FILE="temperature_3d_hybrid_1.dat", STATUS="REPLACE",ACTION="WRITE")
 do i=1,nx

   write(*,*) (T(i,j,1),j=1,end_idx-start_idx+1)
...
!print*,'the itr is',itr
    ...
    !$omp parallel num_threads(threads) private(i,j,k)
       if(my_rank==0) then
        !$omp do collapse(3)
          do i = 2, nx-1
...
OPEN(UNIT=100, FILE="temperature_3d_hybrid.dat", STATUS="REPLACE", ACTION="WRITE")
 do i=1,nx
...
OPEN(UNIT=100, FILE="time_3d_hybrid.dat", STATUS="REPLACE",ACTION="WRITE")
write(100,*) t2-t1

OPEN(UNIT=100, FILE="itr_3d_hybrid.dat", STATUS="REPLACE",ACTION="WRITE")
write(100,*), 'the iterations are',itr
OPEN(UNIT=100, FILE="numprocs_3d_hybrid.dat", STATUS="REPLACE",ACTION="WRITE")
write(100,*), 'the procs are',p
