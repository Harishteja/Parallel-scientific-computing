...
real :: dt,dx,error,error_max,t,t_max,ghost_left,ghost_right,ghost_right2
...
end_idx = (my_rank + 1) * n / p
  ...
  allocate(u(end_idx - start_idx + 2))
  allocate(u_old(end_idx - start_idx + 2))
  allocate(u_exact(end_idx - start_idx + 1))
  allocate(x_position(1000))
  allocate(u_exact_final(1000),u_final(1000))
!print*,'harish'
...
!    call MPI_SEND(u(end_idx - start_idx ),1,MPI_REAL,my_rank+1, 0, MPI_COMM_WORLD, ierr)
!    call MPI_Recv(ghost_left, 1, MPI_REAL, my_rank +1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
!  end if
!  if (my_rank>0) then
!   call MPI_Send(u(1), 1, MPI_REAL, my_rank - 1, 0, MPI_COMM_WORLD, ierr)
!   call MPI_Recv(ghost_right, 1, MPI_REAL, my_rank -1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
!   call MPI_Recv(ghost_right2, 1, MPI_REAL, my_rank -1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
!  end if
  if (my_rank > 0) then
            call MPI_Send(u(1), 1, MPI_REAL, my_rank - 1, 0, MPI_COMM_WORLD, ierr)
!            call MPI_Recv(ghost_left, 1, MPI_REAL, my_rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
...
!  end if
  if (my_rank < num_procs - 1) then
    u(end_idx - start_idx + 2) = ghost_right
  end if
!print*,'please '
!  if (my_rank < num_procs - 1) then
...
u_old=u
u_old(end_idx - start_idx + 2)=u(end_idx - start_idx + 2)
!u_old(0)=u(0)
!u_old(-1)=u(-1)
if (t==0.499770641) then
if(my_rank==1) then
...
u(2)=u_old(2)-(dt/dx)*(u_old(2)-u_old(1))
 do i=3,end_idx - start_idx + 1
   u(i)=u_old(i)-(dt/dx)*((3.0/8.0)*u_old(i)-(7.0/8.0)*u_old(i-1)+(1.0/8.0)*u_old(i-2)+(3.0/8.0)*u_old(i+1))
 end do
 

else if(my_rank==num_procs-1) then
u(1)=u_old(1)-(dt/dx)*((3.0/8.0)*u_old(1)-(7.0/8.0)*ghost_left+(1.0/8.0)*ghost_right2+(3.0/8.0)*u_old(2))
u(2)=u_old(2)-(dt/dx)*((3.0/8.0)*u_old(2)-(7.0/8.0)*u_old(1)+(1.0/8.0)*ghost_left+(3.0/8.0)*u_old(3))
do i=3,end_idx - start_idx 
     u(i)=u_old(i)-(dt/dx)*((3.0/8.0)*u_old(i)-(7.0/8.0)*u_old(i-1)+(1.0/8.0)*u_old(i-2)+(3.0/8.0)*u_old(i+1))
 end do
 u(end_idx-start_idx+1)=0
